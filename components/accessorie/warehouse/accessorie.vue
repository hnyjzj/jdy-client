...
const props = withDefaults(defineProps<{
  filterList: Where<ProductAccessories>
  type: number
}>(), {
  type: 1,
})
const emits = defineEmits<{
   ...
   * @param params 上传入库数据
   */
  upload: [params: ProductAccessories[]]
}>()
...
async function submitGoods() {
  const data: ProductAccessories[] = await transformData(sheetData.value)
  emits('upload', data)
}

/** 下载对应入库模板 */
const downloadLocalFile = () => {
  const url = '/excel/assessoryTemplate.xlsx'
  const name = '配件入库模板.xlsx'
