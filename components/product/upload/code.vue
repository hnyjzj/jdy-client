...
async function transformData(data: any[][]) {
  ...
  // 第一行是表头 对应着字段名
  type ProductKey = keyof BatchCode
  const originalHeaders: ProductKey[] = data[0]

  // 上传后的表头
  const headers = await handleFileUploadCode(originalHeaders) as ProductKey[]
...
async function submitGoods() {
  const data: BatchCode[] = await transformData(sheetData.value)
  emits('upload', data)
}
