...
async function transformData(data: any[]) {
  if (!Array.isArray(data) || data.length < 1) {
    $toast.error('数据格式不正确，至少需要一行数据')
    return []
  }

  const key = Object.keys(data[0])[0] // 假设只需要第一个字段
  return data.slice(1).map(row => ({ code: row[key] })) // 转成你需要的结构
}
async function submitGoods() {
  const data = await transformData(sheetData.value)
  emits('upload', data)
}

/** 下载对应入库模板 */
const downloadLocalFile = () => {
  const url = '/excel/allocateTemplate.xlsx'
...
<template>
  <div>
    <common-model v-model="isModel" title="批量添加" :show-ok="true" confirm-text="导入货品" @confirm="submitGoods" @cancel="clearData">
