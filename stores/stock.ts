export const useStock = defineStore('Stock', {
  ...
  actions: {
     ...
     */
    async getStockTitle() {
      const { data } = await https.get<StockTitle[]>('/statistic/boos/finished_stock/title', undefined, true, false)
      if (data.value?.code === HttpCode.SUCCESS) {
        this.finishedTitle = data.value?.data
      }
    },
    /**
     * 获取成品库存列表
    ...
    async OldGetStockTitle() {
      const { data } = await https.get<StockTitle[]>('/statistic/boos/old_stock/title', undefined, true, false)
      if (data.value?.code === HttpCode.SUCCESS) {
    ...
    async getRevenueTitle() {
      const { data } = await https.get<StockTitle[]>('/statistic/boos/payments/title')
      if (data.value?.code === HttpCode.SUCCESS) {
